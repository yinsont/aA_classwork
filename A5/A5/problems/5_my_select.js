...
function mySelect(arr, cb) {
  ...
  for (let i = 0; i < arr.length; i++) {
    let answer = cb(arr[i], i, arr); // "callback every element, its corresponding index, and the array itself
    if (answer === true) {
      array.push(arr[i]);
    }
  }
  return array; //! REMINDER TO RETURN, cb questions require you to find some thing for cb function that they deal w/
}
